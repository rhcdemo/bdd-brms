...
/drools-reference-implementation/examples/claims/src/test/resources/features
There is only one scenario file, titled claims.feature, in the folder.
...
4. You may view the file in your web browser at:
...
/drools-reference-implementation/examples/claims/src/main/resources/rules
There is only one decision table file, titled SetPrice.xls, in the folder
2. Open the decision table you want to view/edit and add, remove or change the rules as you please
3. Commit your changes back to GitHub
...
NOTE: you may have to request it to build manually
...
And "{name}" is a member of group "{group}"

And "{name}" has visited a "{caregiver}" for "{procedure}" at "{location}"

When determining the claim price for "{name}"

Then "{name}" should be reimbursed {x} dollars

{name} can be any name you want to use
{group} must be one of {default, acme}
{caregiver} must be one of {doctor, nurse practitioner, PA}
{procedure} must be one of {sick visit, well visit, wart removal, EKG}
{location} must be one of {doctor's office, inpatient}
